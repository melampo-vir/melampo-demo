...
public class ForeignSearch extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			//synchronized (MobileCostants.getRootPath()) {
				if (MobileCostants.getRootPath() == null) {
					MobileCostants.setRootPath("http://" + request.getLocalAddr() + ":" + request.getLocalPort() + request.getContextPath()); 

				}
			//}
			
			Parameters advancedOptions = (Parameters) session.getAttribute("advOptions");
	...
	if (parameters == null) {
		...
		session.setAttribute("parameters", parameters);
            ...
            System.out.println(sdf.format( new GregorianCalendar().getTime())  + " - foreign search - request from " + request.getRemoteAddr());
			...
			mobileBean.search(parameters.getXQueryValues(), parameters.getXQueryFields());
		    ...
		    try {
		      ...
		      buf = new BufferedInputStream((new ByteArrayInputStream(xmlResponse.getBytes())));
		      int readBytes = 0;
